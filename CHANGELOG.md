...
- **`osm:aimux` JavaScript module**: provider-agnostic building blocks for agent process management — process registry, PTY spawn, and output parser with pattern-based classification. Used by `osm pr-split` via `require('osm:aimux')`.
