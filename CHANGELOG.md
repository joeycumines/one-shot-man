...
- Go module fork moved from `github.com/dop251/goja` to `github.com/joeycumines/goja` across `go.mod` and all builtins, Go version raised to 1.27.0, and Charm dependencies updated (`bubbles`, `bubbletea`, `lipgloss`, `go-git`, `go-eventloop`, `goja-eventloop`, `mcp/go-sdk`, `x/sys`, `x/term`, `x/text`). Local build helpers (`make-all-with-log`, `make-all-in-container`, `make-all-run-windows`) live in the tracked `example.config.mk` template (copy to the gitignored user-local `config.mk`), and the suite runners (`cross-build`, `test-jscompliance`, `test-jscompliance-all`, `test-test262`, `test-goja-compat`, `fuzz`, `report`, `test-engine`, `cover-engine`) moved to `project.mk`; see `Makefile` and `project.mk`.
