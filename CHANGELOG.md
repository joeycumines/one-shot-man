...
### Added
- **Ollama HTTP client** (`internal/builtin/ollama`): full REST client for Ollama API v0.16.2 — `NewClient(endpoint, ...Option)` with `WithTimeout`, `WithHTTPClient`; endpoints: `Chat` (tool calling), `ChatStream` (streaming), `ListModels`, `ShowModel` (capability detection via `ModelInfo.HasCapability`/`SupportsTools`), `ListRunning`, `Version`, `Health`; comprehensive `ChatRequest`/`ChatResponse`/`Message`/`ToolCall`/`Tool` types; 30+ httptest unit tests
- **Tool calling system** (`tools.go`, `tools_builtin.go`): `ToolDef` with `Handler func(ctx, args) (string, error)`; `ToolRegistry` with `Register`/`MustRegister`/`Get`/`Has`/`Names`/`Len`/`Remove`/`OllamaTools`/`Execute`; 7 built-in tools (`read_file`, `write_file`, `list_dir`, `exec`, `grep`, `git_diff`, `git_log`) registered via `RegisterBuiltinTools(reg, workDir)` with path traversal protection and output truncation
- **Agentic execution loop** (`agent.go`): `AgenticRunner` with `NewAgenticRunner(AgentConfig)` and `Run(ctx, prompt)`/`RunWithMessages(ctx, messages)` — multi-turn tool-calling loop with `MaxTurns` limit, `OnToolCall`/`OnToolResult`/`OnAssistantMessage` callbacks, `FormatToolCallSummary`; returns `AgentResult` with `FinalContent`, `Messages`, `TurnsUsed`, `ToolCallCount`
- **`osm:ollama` JavaScript module** (`module.go`): Promise-based JS bindings — `createClient`, `createToolRegistry`, `registerBuiltinTools`, `createAgent`; agent `run()` returns Promise resolving to `AgentResult`; full tool registration from JS with handler functions
- **OllamaHTTPProvider** (`provider_ollama_http.go`): `Provider` implementation bridging Ollama HTTP client into claude-mux — `Name()="ollama-http"`, `Capabilities={MCP:false, Streaming:true, MultiTurn:true, ModelNav:false}`; `Timeout`, `ToolsEnabled` (`*bool`, nil defaults to true), `ToolsAllowlist` fields; channel-based I/O with `started` guard preventing double-Send panics; 17 unit tests
- 7 Ollama configuration keys: `ollama-endpoint` (default `http://localhost:11434`), `ollama-model`, `ollama-timeout` (default `60s`, validated via `time.ParseDuration`), `ollama-max-turns` (default 10, validated ≥ 1), `ollama-system-prompt`, `ollama-tools-enabled` (default true), `ollama-tools-allowlist` (comma-separated tool names); available as global `claude-mux.ollama-*` and section `[claude-mux] ollama-*` keys with env vars `OSM_OLLAMA_ENDPOINT`, `OSM_OLLAMA_MODEL`
- Tool filtering in OllamaHTTPProvider: when `ollama-tools-enabled` is false, no tools are registered; when `ollama-tools-allowlist` is set, only named tools survive (others removed via `ToolRegistry.Remove()`)
- Config cascade in `resolveProvider()` for ollama-http: CLI flag → ollama-specific config → generic config for model/endpoint/timeout/tools
- Ollama HTTP integration tests (`integration_test.go`): 8 tests against live Ollama server — health check, model listing, model info, single chat, streaming chat, tool calling round-trip, agentic loop, model capabilities; gated by `-integration` flag
- `docs/reference/ollama.md`: comprehensive Ollama HTTP reference covering Client API, ToolRegistry, AgenticRunner, JS module, configuration keys, built-in tools, streaming, and error handling
- `docs/tool-calling.md`: user guide explaining tool calling architecture, built-in tools, Go/JS usage, custom tool registration, tool filtering, model requirements, agentic loop limits, and error handling
...
- PR split rewrite: `orchestrate-pr-split.js` v3.0.0 with claudemux integration (selectStrategy+ChoiceResolver, conflict classification, equivalence verification with diff, createSelectStrategyNode BT leaf) and Ollama AI-powered classification (`classifyChangesWithOllama`, `suggestSplitPlanWithOllama`, `extractJSON`, BT nodes `createOllamaClassifyNode`/`createOllamaPlanNode`/`createOllamaWorkflowTree`)
