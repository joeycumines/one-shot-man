...
| `osm:unicodetext` | Unicode text utilities | `width(s) → number` (monospace display width), `truncate(s, maxWidth, tail?) → string`, `padLeft(s, width, pad?) → string`, `padRight(s, width, pad?) → string`, `padCenter(s, width, pad?) → string`. [Reference →](reference/unicodetext.md) |
...
| Module | Description | Key exports |
|--------|-------------|-------------|
| `osm:bubbletea` | BubbleTea v2 TUI framework bindings | `newModel(config) → Model`, `run(model, opts?)` (non-blocking), `waitForProgram() → Promise<void>` (resolves when the program started by `run` has fully exited — call it from an async chain AFTER `run`, before starting the next program; a bare top-level call in a synchronous script can race the engine's own post-script wait on the same slot); `isTTY() → bool`; Commands: `quit()`, `clearScreen()`, `batch(…cmds)`, `sequence(…cmds)`, `tick(ms, id?)`, `requestWindowSize()`; Metadata: `keys`, `keysByName`, `mouseButtons`; Validation: `isValidTextareaInput(key)`, `isValidLabelInput(key)` |
