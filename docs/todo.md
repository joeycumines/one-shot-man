...
- **PARKED (2026-08-24, per directive — consensus engine not in scope, stripped from blueprint.json): Blinded Dual-Auditor Consensus Engine & Multi-Model Router + Interactive TUI Verification Pane — FULL CONTEXT PRESERVED BELOW, strictly within docs/todo.md**
    - **Original Task 1: Build Blinded Dual-Auditor Consensus Engine & Multi-Model Router (tail) — Status: Not Started — DELETED from blueprint.json**
        - Description: Implement orchestration on the new contract. Prerequisite: Core Engine complete. Unit 1: Create internal/builtin/llmrouter/llmrouter.go exposing provider-agnostic routing (OpenRouter, Anthropic, OpenAI, DeepSeek, Google, local vLLM/SGLang) with cache_control, Batch API 24h SLA, sticky routing, ZDR headers — all via Adapter.NewPromise+Settler (no Loop.Promisify). Unit 2: Create embedded JS orchestration in internal/command/code_review_3stage.js: Stage 1 micro-diff triage via cheap model; Stage 2 astpack; Stage 3 blinded dual-auditing (Open-Weights Red Teamer Temp 0.4 vs Closed Verifier Temp 0.1); Consensus Engine normalizing SARIF, computing line overlap +/-5, auto-blocking unanimous, routing disputes to Stage 3C Arbiter. Unit 3: Update CodeReviewCommand with --3stage etc. Unit 4: Tests in code_review_3stage_test.go and llmrouter_test.go.
        - Acceptance: osm code-review --3stage executes against mock LLM servers, generating SARIF and Markdown. Consensus auto-accepts unanimous bugs, discards low-severity, routes disputes to Arbiter. Batch+Cache stacking validated on mock endpoints, 80%+ cost reduction. Tests pass with -race.
        - Files: internal/builtin/llmrouter/llmrouter.go, internal/builtin/llmrouter/module.go, internal/builtin/llmrouter/llmrouter_test.go, internal/command/code_review_3stage.js, internal/command/code_review_3stage.go, internal/command/code_review_3stage_test.go, internal/command/code_review.go
        - ContextPointers: scriptPrecedent=internal/command/code_review_script.js, prSplitPipeline=internal/command/pr_split_10d_pipeline_orchestrator.js
        - DependsOn: Build 3-Stage Pipeline Core Engine: Go AST Packager, Micro-Diff Filter, and JS Bindings (tail, on new contract); Complete JS Compliance Suite remainder: CommonJS require cache/resolve and Web-API gap pinning (tail, now on new surface)
        - Deleted code: internal/builtin/llmrouter/ (entire directory, 2 files llmrouter.go + module.go of 250+141 lines) was created as scaffold for this task and has been `rm -rf` per directive — zero remaining references (grep -r llmrouter returns only scratch logs)
    - **Original Task 2: Build Interactive TUI Verification Pane & End-to-End Compliance Suite (tail) — Status: Not Started — DELETED from blueprint.json**
        - Description: Implement UI and integration on the new surface. Prereq: integration green and Core Engine complete. Unit 1: Create internal/command/code_review_tui.js rendering BubbleTea/TermMux TUI with real-time 3-stage progress, AST diff trees, dual-auditor side-by-side, arbitration logs — via owner-safe adapter bindings. Unit 2: Create internal/jscompliance/specs/code_review_3stage.spec.js verifying osm:astpack/diff_triage/llmrouter bindings. Unit 3: Build cross-platform E2E in code_review_e2e_test.go (macOS, make-all-in-container, make-all-run-windows). Unit 4: Update docs/scripting.md and README.md with CLI flags, architecture diagrams, JS module exports.
        - Acceptance: gmake test-jscompliance-all green on all platforms. TUI renders dual-auditor panes without key-handling crashes or loop blocking. Docs accurately reflect CLI flags and JS exports. Charts pass.
        - Files: internal/command/code_review_tui.js, internal/command/code_review_e2e_test.go, internal/jscompliance/specs/code_review_3stage.spec.js, docs/scripting.md, README.md
        - ContextPointers: tuiPrecedent=internal/command/pr_split_13_tui.js, compliancePrecedent=internal/jscompliance/specs/modules_pure.spec.js
        - DependsOn: Realign AGENTS.md with the August 2026 integration surface (preserve intent, update contract); Enforce zero-legacy grep gate and run cross-platform build; Build 3-Stage Pipeline Core Engine: Go AST Packager, Micro-Diff Filter, and JS Bindings (tail, on new contract)
        - Deleted from blueprint: also removed from final Rule-of-Two dependsOn list (previously included both tasks, now pruned)
    - **Stripping details**: blueprint.json currentState updated from “Next: T23 Dual-Auditor/LLM Router + T24 TUI + T25 gap sweep” to “Consensus engine STRIPPED per directive — parked in docs/todo.md. Next: T25 gap sweep + T26 Rule of Two.”; goalState trimmed to remove “production-grade 3-Stage pipeline remains the tail goal” clause; sequentialTasks reduced from 6 remaining (including T23/T24) to 4 remaining; zero code remains — llmrouter dir deleted, no 3stage files ever created, register.go never wired llmrouter.
    - **Re-entry criteria**: If consensus engine is re-scoped, re-add tasks to blueprint.json with same file lists above, re-create internal/builtin/llmrouter/ via async.Promise precedent (see internal/builtin/astpack/module.go), and re-wire register.go per internal/builtin/register.go:125-126 pattern. All context above is canonical.
