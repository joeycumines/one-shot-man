// Package aimuxcore provides a PTY output parser for classifying AI terminal
// output into typed events. It detects rate limits, permission prompts,
...
// NewParser creates a parser pre-loaded with AI terminal output patterns.
...
func (p *Parser) AddPattern(name string, pattern string, eventType EventType) error {
	...
	if err != nil {
		return fmt.Errorf("aimux: invalid pattern %q: %w", name, err)
	}
...
// loadBuiltinPatterns registers the default AI terminal output patterns.
