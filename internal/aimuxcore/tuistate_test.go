...
func TestNewTUIStateMachine(t *testing.T) {
	sm, err := NewTUIStateMachine(DefaultTUIStateConfig())
...
func TestDefaultTUIStateConfig(t *testing.T) {
	config := DefaultTUIStateConfig()

	assert.Len(t, config.ReadyPatterns, 1)
	assert.Len(t, config.ProcessingPatterns, 2)
	...
	assert.Contains(t, config.ProcessingPatterns, `(?i)(thinking|analyzing|processing|working)\.\.\.`)
