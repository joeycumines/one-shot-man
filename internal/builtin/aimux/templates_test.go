...
// templateTestEnv sets up a full JS environment with osm:bt, osm:aimux,
...
func templateTestEnv(t *testing.T) (*btmod.Bridge, func(string) goja.Value) {
	...
	if runtime.GOOS == "windows" {
		t.Skip("aimux templates use sh -c; skipping on Windows")
	}
	...
	reg.RegisterNativeModule("osm:exec", execmod.Require(ctx, nil, loop))
