...
func asyncEnv(t *testing.T) (*goja.Runtime, func(string) (goja.Value, error)) {
	t.Helper()
	if testing.Short() {
		t.Skip("skip async astpack test in -short")
	}
	rt := goja.New()
...
func TestAstpackPackReturnsPromise(t *testing.T) {
	_, runJS := asyncEnv(t)
	v, err := runJS(`astpack.pack({ "foo.go": "package foo\nfunc Foo(){ Bar() }\nfunc Bar(){}\n" }).then(function(x){ __collect(JSON.stringify(x)); });`)
	if err != nil {
		t.Fatalf("pack promise: %v", err)
	}
	if v.String() == "" {
		t.Fatalf("expected json string")
	}
}

func TestAstpackPackDiffValue(t *testing.T) {
	_, runJS := asyncEnv(t)
	v, err := runJS(`astpack.packDiff("diff --git a/foo.go b/foo.go\n--- a/foo.go\n+++ b/foo.go\n@@ -1 +1 @@\n package foo\n+func Added(){}\n").then(function(pkg){ __collect(JSON.stringify(pkg)); });`)
	if err != nil {
		t.Fatalf("packDiff: %v", err)
	}
	if v.String() == "" {
