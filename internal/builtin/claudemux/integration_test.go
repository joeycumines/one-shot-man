...
// ============================================================================
// MCP Result Channel Integration Tests
// ============================================================================

// TestIntegration_MCPToolRoundTrip performs a real end-to-end MCP tool call
// round-trip:
//  1. Create MCPInstanceConfig with result-dir
//  2. Spawn Claude Code via the configured provider
//  3. Send a classification prompt referencing the MCP tool
//  4. Wait for Claude Code to call reportClassification
//  5. Verify the result file was written and is parseable
//
// Prerequisites: -integration flag + real provider (ollama or claude-code).
func TestIntegration_MCPToolRoundTrip(t *testing.T) {
	skipIfNotIntegration(t)

	prov := resolveTestProvider(t)
	if !prov.Capabilities().MCP {
		t.Skipf("provider %q does not support MCP", prov.Name())
	}

	// Create result directory.
	resultDir := filepath.Join(t.TempDir(), "results")

	// Create MCP instance config.
	mcpCfg, err := NewMCPInstanceConfig("mcp-round-trip")
	if err != nil {
		t.Fatalf("NewMCPInstanceConfig: %v", err)
	}
	defer func() { _ = mcpCfg.Close() }()

	mcpCfg.ResultDir = resultDir
	if err := mcpCfg.WriteConfigFile(); err != nil {
		t.Fatalf("WriteConfigFile: %v", err)
	}

	t.Logf("MCP config: %s", mcpCfg.ConfigPath())
	t.Logf("Result dir: %s", resultDir)

	// Spawn with MCP config args.
	ctx, cancel := context.WithTimeout(context.Background(), 120*time.Second)
	defer cancel()

	spawnArgs := mcpCfg.SpawnArgs()
	t.Logf("Spawn args: %v", spawnArgs)
	...
	// Wait for Claude Code to be ready (initial output).
	readyOutput := collectUntil(t, handle, 30*time.Second, func(acc string) bool {
		return len(acc) > 100
	})
	t.Logf("Ready output (%d bytes)", len(readyOutput))

	// Send classification prompt.
	prompt := `Classify each file into a category using the MCP tool "reportClassification".` +
		` Use sessionId "mcp-round-trip". Files: cmd/main.go, internal/foo.go, docs/README.md` +
		` Categories: entry-point, impl, docs\n`

	if err := handle.Send(prompt); err != nil {
		t.Fatalf("Send prompt: %v", err)
	}

	// Wait for completion — either result file appears or agent completes.
	var resultData []byte
	...
	for {
		select {
		case <-deadline:
			t.Fatal("timeout waiting for classification result file")
		case <-ticker.C:
			data, readErr := os.ReadFile(filepath.Join(resultDir, "classification.json"))
			if readErr == nil && len(data) > 2 {
				resultData = data
				goto done
			}
		}
	...
	t.Logf("Result file written (%d bytes): %s", len(resultData), string(resultData))

	// Parse and verify.
	result, err := ReadClassificationResult(resultDir)
	if err != nil {
		t.Fatalf("ReadClassificationResult: %v", err)
	}

	if len(result) == 0 {
		t.Fatal("classification result is empty")
	}

	t.Logf("Classification result: %d files", len(result))
	for path, cat := range result {
		t.Logf("  %s → %s", path, cat)
	}

	// Verify all 3 files were classified.
	expectedFiles := []string{"cmd/main.go", "internal/foo.go", "docs/README.md"}
	for _, f := range expectedFiles {
		if _, ok := result[f]; !ok {
			t.Errorf("file %q not in classification result", f)
		}
	}
}

// TestIntegration_SplitPlanRoundTrip performs a real end-to-end MCP
// reportSplitPlan tool call:
//  1. Spawn Claude Code with MCP config
//  2. Send a planning prompt
//  3. Wait for Claude Code to call reportSplitPlan
//  4. Verify the result file is parseable
func TestIntegration_SplitPlanRoundTrip(t *testing.T) {
	skipIfNotIntegration(t)
	...
	mcpCfg, err := NewMCPInstanceConfig("mcp-plan-trip")
	if err != nil {
		t.Fatalf("NewMCPInstanceConfig: %v", err)
	}
	defer func() { _ = mcpCfg.Close() }()
	...
	prompt := `Create a PR split plan using the MCP tool "reportSplitPlan".` +
		` Use sessionId "mcp-plan-trip".` +
		` Files: pkg/types.go, pkg/impl.go, pkg/impl_test.go, cmd/main.go, docs/readme.md` +
		` Create stages: types (pkg/types.go), impl (pkg/impl.go, pkg/impl_test.go), cmd (cmd/main.go), docs (docs/readme.md)` +
	...
	for {
		select {
		case <-deadline:
			t.Fatal("timeout waiting for split-plan result file")
		case <-ticker.C:
			data, readErr := os.ReadFile(filepath.Join(resultDir, "split-plan.json"))
			if readErr == nil && len(data) > 2 {
				goto planDone
			}
		}
...
planDone:
	...
	result, err := ReadSplitPlanResult(resultDir)
	if err != nil {
		t.Fatalf("ReadSplitPlanResult: %v", err)
	}
	if len(result) == 0 {
		t.Fatal("split plan result is empty")
	}

	t.Logf("Split plan: %d stages", len(result))
	for _, stage := range result {
		t.Logf("  stage %d: %s (%d files) — %s", stage.Order, stage.Name, len(stage.Files), stage.Message)
	}

	// Verify every file appears in exactly one stage.
	allFiles := make(map[string]bool)
	for _, stage := range result {
		for _, f := range stage.Files {
			if allFiles[f] {
				t.Errorf("duplicate file %q across stages", f)
			}
			allFiles[f] = true
		}
	}
	if len(allFiles) < 3 {
		t.Errorf("expected at least 3 files in plan, got %d", len(allFiles))
	}
}

// TestIntegration_FullPRSplitWorkflow is the FLAGSHIP integration test.
// It creates a test git repository, runs the full AI-powered PR split
// workflow through the claudemux BT tree, and verifies:
//   - Branches are created
//   - Each branch compiles (via verifyCommand)
//   - Tree hash equivalence holds
//
// This test requires real agent infrastructure and takes 2-5 minutes.
func TestIntegration_FullPRSplitWorkflow(t *testing.T) {
	skipIfNotIntegration(t)
	...
	dir := t.TempDir()
	runIntegrationGit(t, dir, "init", "-b", "main")
	runIntegrationGit(t, dir, "config", "user.email", "integration@test.com")
	runIntegrationGit(t, dir, "config", "user.name", "Integration Test")
	...
	runIntegrationGit(t, dir, "add", "-A")
	runIntegrationGit(t, dir, "commit", "-m", "initial commit")

	// Feature branch with changes across several directories.
	runIntegrationGit(t, dir, "checkout", "-b", "feature-split")
	for _, f := range []struct{ path, content string }{
		{"pkg/impl.go", "package pkg\n\nfunc NewUser(name string) User { return User{Name: name} }\n"},
		{"pkg/impl_test.go", "package pkg\n\nimport \"testing\"\n\nfunc TestNewUser(t *testing.T) {\n\tu := NewUser(\"test\")\n\tif u.Name != \"test\" { t.Fatal() }\n}\n"},
	...
	runIntegrationGit(t, dir, "add", "-A")
	runIntegrationGit(t, dir, "commit", "-m", "feature: add user implementation and docs")

	t.Logf("Test repo ready at %s", dir)

	// Set up the MCP instance for the classification step.
	resultDir := filepath.Join(t.TempDir(), "mcp-results")
	mcpCfg, err := NewMCPInstanceConfig("full-workflow")
	if err != nil {
		t.Fatalf("NewMCPInstanceConfig: %v", err)
	}
	defer func() { _ = mcpCfg.Close() }()
	...
	// Spawn agent for classification.
	...
	if err != nil {
		t.Fatalf("Spawn classification: %v", err)
	}

	// Wait for ready.
	collectUntil(t, handle, 30*time.Second, func(acc string) bool {
		return len(acc) > 100
	})

	// Send classification prompt.
	classifyPrompt := `You are a PR splitting assistant. Classify these changed files:` +
		` pkg/impl.go, pkg/impl_test.go, cmd/run.go, docs/guide.md` +
		` Use the MCP tool "reportClassification" with sessionId "full-workflow".` +
		` Categories: impl, tests, cmd, docs\n`
	if err := handle.Send(classifyPrompt); err != nil {
		t.Fatalf("Send classify: %v", err)
	}

	// Wait for classification result.
	waitForResultFile(t, filepath.Join(resultDir, "classification.json"), 90*time.Second)
	classification, err := ReadClassificationResult(resultDir)
	if err != nil {
		t.Fatalf("ReadClassificationResult: %v", err)
	}
	t.Logf("Classification: %v", classification)
	handle.Close()

	// Now spawn for planning (fresh agent).
	planResultDir := filepath.Join(t.TempDir(), "plan-results")
	planCfg, err := NewMCPInstanceConfig("full-workflow-plan")
	if err != nil {
		t.Fatalf("NewMCPInstanceConfig plan: %v", err)
	}
	defer func() { _ = planCfg.Close() }()
	planCfg.ResultDir = planResultDir
	if err := planCfg.WriteConfigFile(); err != nil {
		t.Fatalf("WriteConfigFile plan: %v", err)
	}

	handle2, err := prov.Spawn(ctx, SpawnOpts{
		Model: testModel,
		Dir:   dir,
		Rows:  24,
		Cols:  120,
		Args:  planCfg.SpawnArgs(),
	})
	if err != nil {
		t.Fatalf("Spawn plan: %v", err)
	}

	collectUntil(t, handle2, 30*time.Second, func(acc string) bool {
		return len(acc) > 100
	})

	planPrompt := `Create a PR split plan using "reportSplitPlan" tool with sessionId "full-workflow-plan".` +
		` Files: pkg/impl.go, pkg/impl_test.go, cmd/run.go, docs/guide.md` +
		` Stage 1 (order 0): "impl" with pkg/impl.go, pkg/impl_test.go, message "feat: add user implementation"` +
		` Stage 2 (order 1): "cmd" with cmd/run.go, message "feat: add run command"` +
		` Stage 3 (order 2): "docs" with docs/guide.md, message "docs: add user guide"\n`
	if err := handle2.Send(planPrompt); err != nil {
		t.Fatalf("Send plan: %v", err)
	}

	waitForResultFile(t, filepath.Join(planResultDir, "split-plan.json"), 90*time.Second)
	planResult, err := ReadSplitPlanResult(planResultDir)
	if err != nil {
		t.Fatalf("ReadSplitPlanResult: %v", err)
	}
	t.Logf("Split plan: %d stages", len(planResult))
	for _, s := range planResult {
		t.Logf("  stage %d: %s → %v", s.Order, s.Name, s.Files)
	}
	handle2.Close()

	if len(planResult) < 2 {
		t.Fatalf("expected at least 2 stages, got %d", len(planResult))
	}

	t.Log("FLAGSHIP: Full PR split workflow completed successfully")
}

// waitForResultFile polls for a file to appear with content.
func waitForResultFile(t *testing.T, path string, timeout time.Duration) {
	t.Helper()
	deadline := time.After(timeout)
	ticker := time.NewTicker(500 * time.Millisecond)
	defer ticker.Stop()
	for {
		select {
		case <-deadline:
			t.Fatalf("timeout waiting for result file: %s", path)
		case <-ticker.C:
...
// runIntegrationGit executes a git command for integration tests.
