...
func Require(manager *Manager) func(runtime *goja.Runtime, module *goja.Object) {
	if manager == nil {
		...
		if err != nil {
			panic("lipgloss.Require: " + err.Error())
		}
	}
	return func(runtime *goja.Runtime, module *goja.Object) {
		...
		_ = exports.Set("setDarkBackground", func(call goja.FunctionCall) goja.Value {
			if len(call.Arguments) > 0 {
				dark := call.Argument(0).ToBoolean()
				if manager.detectedDarkBackground == nil {
					manager.detectedDarkBackground = new(bool)
				}
				*manager.detectedDarkBackground = dark
			}
			return goja.Undefined()
		})
	}
