// Command mockclaude simulates Claude Code's NDJSON protocol interface.
//
...
// protocol mode integration without needing real Claude Code.
//
...
// JSON field names use camelCase to match the ClaudeEvent struct in
...
func main() {
	...
	if err != nil {
		_, _ = fmt.Fprintf(os.Stderr, "mockclaude: %v\n", err)
		os.Exit(1)
	}
}
...
func writeMsg(out *bufio.Writer, msg outbound) {
	...
	if err != nil {
		_, _ = fmt.Fprintf(os.Stderr, "mockclaude: marshal error: %v\n", err)
		return
	}
	_, _ = out.Write(data)
	_ = out.WriteByte('\n')
