...
func TestCodeReviewCommand_ShowActualDiffOutput(t *testing.T) {
	...
	// Isolate the lazy-diff execution from the live repository: stub the git
	// diff runner with a small deterministic diff. The test verifies prompt
	// assembly (template text, lazy-diff markers, item listing), not the git
	// CLI itself, and a live `git diff` under parallel load can stall past
	// the async-completion budget below.
