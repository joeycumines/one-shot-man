...
// ==========================================================================
// claude_mux.go — resolveProvider (58.3%)
// ==========================================================================

// TestResolveProvider_OllamaDefault covers the "ollama" provider case.
func TestResolveProvider_OllamaDefault(t *testing.T) {
	t.Parallel()
	cfg := config.NewConfig()
	cmd := NewClaudeMuxCommand(cfg)
	cmd.runProvider = "ollama"
...
// TestResolveProvider_OllamaWithCommand covers the "ollama" provider with custom
// command override.
func TestResolveProvider_OllamaWithCommand(t *testing.T) {
	t.Parallel()
	cfg := config.NewConfig()
	cmd := NewClaudeMuxCommand(cfg)
	cmd.runProvider = "ollama"
	cmd.runCommand = "/custom/ollama"
...
// TestResolveProvider_ClaudeCodeWithCommand covers "claude-code" with custom
// command override.
func TestResolveProvider_ClaudeCodeWithCommand(t *testing.T) {
	t.Parallel()
	cfg := config.NewConfig()
	cmd := NewClaudeMuxCommand(cfg)
	cmd.runProvider = "claude-code"
	cmd.runCommand = "/usr/local/bin/claude"
...
// TestResolveProvider_UnknownProvider returns an error for unknown providers.
func TestResolveProvider_UnknownProvider(t *testing.T) {
	t.Parallel()
	cfg := config.NewConfig()
	cmd := NewClaudeMuxCommand(cfg)
	cmd.runProvider = "unknown-provider"

	_, err := cmd.resolveProvider()
	require.Error(t, err)
	assert.Contains(t, err.Error(), "unknown provider")
	assert.Contains(t, err.Error(), "unknown-provider")
}

// TestResolveProvider_EmptyStringIsClaudeCode covers the "" case that
// defaults to ClaudeCode.
func TestResolveProvider_EmptyStringIsClaudeCode(t *testing.T) {
	t.Parallel()
	cfg := config.NewConfig()
	cmd := NewClaudeMuxCommand(cfg)
	cmd.runProvider = ""
