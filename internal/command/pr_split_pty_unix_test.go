...
//
// The go-prompt sync protocol is enabled at runtime via the
// OSM_SYNC_PROTOCOL=1 environment variable (set by the test harness).
// Without this, go-prompt processes multiple bytes arriving in a single
// PTY read as a single key event — causing input like "auto-split\r" to
// be treated as unrecognised text rather than a command followed by
// Enter. The sync protocol, combined with termtest.Console's
