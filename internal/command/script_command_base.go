...
func (b *scriptCommandBase) PrepareEngine(ctx context.Context, stdout, stderr io.Writer) (*scripting.Engine, func(), error) {
	...
	engine, err := scripting.NewEngine(ctx, stdout, stderr, b.session, b.store, lc.logFile, lc.bufferSize, lc.level, append(modulePathOpts(b.config), userK8sOpts(b.config)...)...)
	if err != nil {
		if lc.logFile != nil {
			lc.logFile.Close()
		}
