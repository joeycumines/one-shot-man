...
// switchTab sends Tab and waits for the expected view name in the status bar.
func switchTab(t *testing.T, ctx context.Context, cp *termtest.Console, viewName string) {
	t.Helper()
	target := "View: " + viewName
	// Check if already on the target view
	if containsStr(cp.String(), target) {
		return
	}
	// Send Tab and wait for the view to switch
	snap := cp.Snapshot()
	sendKey(t, cp, "\t")
	ctx2, cancel := context.WithTimeout(ctx, 3*time.Second)
	defer cancel()
	cp.Expect(ctx2, snap, termtest.Contains(target), "wait for "+target)
...
func TestDashboard_InitialRender(t *testing.T) {
	...
	buf := cp.String()
	assert.Contains(t, buf, "DARK")
	assert.Contains(t, buf, "View: Overview")
	assert.Contains(t, buf, "CPU")
...
func TestDashboard_OverviewMetrics(t *testing.T) {
	...
	buf := cp.String()
	assert.Contains(t, buf, "CPU")
	assert.Contains(t, buf, "MEM")
	assert.Contains(t, buf, "NET")
	assert.Contains(t, buf, "Processes")
	assert.Contains(t, buf, "\xe2\x96\x81")
	assert.Contains(t, buf, "PID")
...
func TestDashboard_ShowcaseNavigation(t *testing.T) {
	...
	buf := cp.String()
	assert.Contains(t, buf, "Box")
	assert.Contains(t, buf, "Label")
	assert.Contains(t, buf, "Divider")
	assert.Contains(t, buf, "List")
	assert.Contains(t, buf, "Viewport")
	assert.Contains(t, buf, "Textarea")
...
func TestDashboard_TabSwitching(t *testing.T) {
	...
	for _, v := range views {
		...
		time.Sleep(300 * time.Millisecond)
		buf := cp.String()
		assert.Contains(t, buf, v.content, "view %s should show %s", v.name, v.content)
	}
...
func TestDashboard_ComposeInteract(t *testing.T) {
	...
	time.Sleep(300 * time.Millisecond)
	buf := cp.String()
	assert.Contains(t, buf, "Composition Editor")
	assert.Contains(t, buf, "Main")

	sendKey(t, cp, "n")
	time.Sleep(300 * time.Millisecond)
	buf = cp.String()
	assert.Contains(t, buf, "P3")
...
func TestDashboard_AgentsInteract(t *testing.T) {
	...
	time.Sleep(300 * time.Millisecond)
	buf := cp.String()
	assert.Contains(t, buf, "Planner")
	assert.Contains(t, buf, "Coder")
	assert.Contains(t, buf, "Reviewer")
	assert.Contains(t, buf, "Tester")
...
func TestDashboard_BuilderInteract(t *testing.T) {
	...
	time.Sleep(300 * time.Millisecond)
	buf := cp.String()
	assert.Contains(t, buf, "Layout Builder")
	assert.Contains(t, buf, "split")

	sendKey(t, cp, "g")
	time.Sleep(200 * time.Millisecond)
	buf = cp.String()
	assert.Contains(t, buf, "grid")

	sendKey(t, cp, "t")
	time.Sleep(200 * time.Millisecond)
	buf = cp.String()
	assert.Contains(t, buf, "stack")

	sendKey(t, cp, "h")
	time.Sleep(200 * time.Millisecond)
	buf = cp.String()
	assert.Contains(t, buf, "horizontal")

	sendKey(t, cp, "v")
	time.Sleep(200 * time.Millisecond)
	buf = cp.String()
	assert.Contains(t, buf, "vertical")

	sendKey(t, cp, "+")
	time.Sleep(200 * time.Millisecond)
	buf = cp.String()
	assert.Contains(t, buf, "C")
...
func TestDashboard_LogViewer(t *testing.T) {
	...
	time.Sleep(300 * time.Millisecond)
	buf := cp.String()
	assert.Contains(t, buf, "Live Log Viewer")
	assert.Contains(t, buf, "osm-engine")
...
func TestDashboard_EditorInteract(t *testing.T) {
	...
	time.Sleep(300 * time.Millisecond)
	buf := cp.String()
	assert.Contains(t, buf, "Text Editor")
	assert.Contains(t, buf, "Chars:")

	sendKey(t, cp, "f")
	time.Sleep(200 * time.Millisecond)
	buf = cp.String()
	assert.Contains(t, buf, "FOCUSED")
...
func TestDashboard_HelpView(t *testing.T) {
	...
	time.Sleep(300 * time.Millisecond)
	buf := cp.String()
	assert.Contains(t, buf, "Keyboard Shortcuts")
	assert.Contains(t, buf, "Tab")
	assert.Contains(t, buf, "Compose")
	assert.Contains(t, buf, "Agents")
...
func TestDashboard_ModalDemo(t *testing.T) {
	...
	time.Sleep(500 * time.Millisecond)
	buf := cp.String()
	assert.Contains(t, buf, "Modal dialog")
...
func TestDashboard_ToastDemo(t *testing.T) {
	...
	time.Sleep(500 * time.Millisecond)
	buf := cp.String()
	assert.Contains(t, buf, "Box demo activated")
