...
func TestMain(m *testing.M) {
	...
	// Walk up from internal/example/dashboard to find the repo root.
	// The repo root contains cmd/osm/main.go.
	repoRoot := findRepoRoot(wd)

	tmpBase := os.TempDir()
	testBinaryDir = filepath.Join(tmpBase, fmt.Sprintf("osm-dashboard-test-%d", os.Getpid()))
...
// findRepoRoot walks up from dir until it finds cmd/osm/main.go.
