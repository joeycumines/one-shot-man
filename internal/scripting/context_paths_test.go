...
func TestAddPathExpandsTilde(t *testing.T) {
	// Set up a fake HOME directory before defining test cases to ensure
	// hermetic test execution.
	fakeHome := t.TempDir()
	t.Setenv("HOME", fakeHome)
	t.Setenv("USERPROFILE", fakeHome) // Windows compat
	...
	t.Run("Unix tilde syntax", func(t *testing.T) {
		unixTestCases := []struct {
			name     string
			input    string
			expected string // expected output ("" means same as input)
		}{
			...
			{"no tilde", "foo/bar.txt", ""},
			{"tilde only", "~", fakeHome},       // bare ~ expands to home dir
			{"tilde with spaces", "~ /foo", ""}, // not a valid tilde path
		}
	...
	t.Run("Windows tilde syntax", func(t *testing.T) {
		if runtime.GOOS != "windows" {
			t.Skip("Windows-specific tilde syntax tests")
		}
		...
		windowsTestCases := []struct {
			...
			{"relative path windows", ".\\foo\\bar.txt", ""},
			{"absolute path windows", "C:\\tmp\\foo.txt", ""},
			{"no tilde windows", "foo\\bar.txt", ""},
			{"tilde only windows", "~", fakeHome},        // bare ~ expands to home dir
			{"tilde with spaces windows", "~ \\foo", ""}, // not a valid tilde path
		}
	...
	// Actually test AddPath with a tilde path using a fake HOME to avoid
...
func TestAddPathBareTilde(t *testing.T) {
	fakeHome := t.TempDir()
	t.Setenv("HOME", fakeHome)
	t.Setenv("USERPROFILE", fakeHome)
	...
	homeFile := filepath.Join(testDir, "home_file.txt")
	...
	// The directory contents should be tracked relative to the base path.
	logical := filepath.Join("testsub", "home_file.txt")
	cp, ok := cm.GetPath(logical)
	if !ok {
		t.Fatalf("expected %q to be tracked after AddPath(\"~\"), paths: %v", logical, cm.ListPaths())
	}
	if cp.Content != "home content" {
		t.Errorf("expected content %q, got %q", "home content", cp.Content)
	}
}
...
func TestExpandTildeErrorWhenHomeDirUnavailable(t *testing.T) {
	...
	//
	// Note: This test is best-effort. On some systems, os.UserHomeDir has
	// fallback mechanisms (e.g., reading /etc/passwd on Unix) that may still
	// succeed even with unset environment variables.
	...
	// If os.UserHomeDir failed, verify the error message
	if err != nil {
		// Error is expected on systems where os.UserHomeDir fails
	...
	// If os.UserHomeDir succeeded (system has fallback), document this
	// and verify the result is still a valid path
	t.Logf("os.UserHomeDir succeeded on this system even with unset env vars")
	if result != "" && result != "~/test.txt" {
		// If expansion succeeded, the result should be an absolute path
...
func TestTildePathRoundTrip(t *testing.T) {
	// Test that tilde paths work consistently across AddPath, RefreshPath, and RemovePath
	fakeHome := t.TempDir()
	t.Setenv("HOME", fakeHome)
	t.Setenv("USERPROFILE", fakeHome)
	...
	testDir := filepath.Join(fakeHome, "roundtrip")
	if err := os.MkdirAll(testDir, 0o755); err != nil {
	...
	t.Run("AddPath with tilde", func(t *testing.T) {
		// Add the file using a tilde path
		if err := cm.AddPath("~/roundtrip/test.txt"); err != nil {
			t.Fatalf("AddPath with tilde failed: %v", err)
		}

		// Verify the file is tracked
		logical := filepath.Join("roundtrip", "test.txt")
		cp, ok := cm.GetPath(logical)
		if !ok {
			t.Fatalf("expected %q to be tracked, paths: %v", logical, cm.ListPaths())
		}
		if cp.Content != "initial content" {
			t.Errorf("expected content %q, got %q", "initial content", cp.Content)
		}
	})

	t.Run("RefreshPath with tilde", func(t *testing.T) {
		// Update the file on disk
		if err := os.WriteFile(testFile, []byte("refreshed content"), 0o644); err != nil {
			t.Fatalf("failed to update test file: %v", err)
		}

		// Refresh using a tilde path
		if err := cm.RefreshPath("~/roundtrip/test.txt"); err != nil {
			t.Fatalf("RefreshPath with tilde failed: %v", err)
		}
		...
		cp, ok := cm.GetPath(logical)
		if !ok {
			t.Fatalf("expected %q to be tracked after refresh", logical)
		}
		if cp.Content != "refreshed content" {
			t.Errorf("expected content %q after refresh, got %q", "refreshed content", cp.Content)
		}
	})

	t.Run("RemovePath with tilde", func(t *testing.T) {
		// Remove using a tilde path
		if err := cm.RemovePath("~/roundtrip/test.txt"); err != nil {
			t.Fatalf("RemovePath with tilde failed: %v", err)
		}

		// Verify the file is no longer tracked
		logical := filepath.Join("roundtrip", "test.txt")
		if _, ok := cm.GetPath(logical); ok {
			t.Fatalf("expected %q to be removed after RemovePath with tilde", logical)
		}
	})
}
