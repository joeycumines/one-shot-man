...
// TestToTxtar_PathDisambiguation verifies all 3 scenarios from the txtar path
// ambiguity bug: LCA computation, collision resolution, and non-colliding paths.
func TestToTxtar_PathDisambiguation(t *testing.T) {
	t.Parallel()

	t.Run("LCA_emitted", func(t *testing.T) {
		t.Parallel()
		tmpDir := t.TempDir()

		// Create files under a common ancestor: src/a/foo.go and src/b/bar.go.
		for _, rel := range []string{"src/a/foo.go", "src/b/bar.go"} {
			full := filepath.Join(tmpDir, rel)
			if err := os.MkdirAll(filepath.Dir(full), 0755); err != nil {
				t.Fatal(err)
			}
			if err := os.WriteFile(full, []byte("pkg "+filepath.Base(rel)), 0644); err != nil {
				t.Fatal(err)
			}
		}
		...
		cm, _ := NewContextManager(tmpDir)
		for _, rel := range []string{"src/a/foo.go", "src/b/bar.go"} {
			if err := cm.AddPath(filepath.Join(tmpDir, rel)); err != nil {
				t.Fatal(err)
			}
		}
		...
		// Verify LCA is emitted in the comment.
		if !strings.Contains(result, "common path: src") {
			t.Errorf("expected 'common path: src' in output, got:\n%s", result)
		}

		// Both files should appear with full relative paths.
		if !strings.Contains(result, "src/a/foo.go") {
			t.Errorf("expected 'src/a/foo.go' in output, got:\n%s", result)
		}
		if !strings.Contains(result, "src/b/bar.go") {
			t.Errorf("expected 'src/b/bar.go' in output, got:\n%s", result)
		}
	})

	t.Run("CollidingBasenamesPreserveFullPath", func(t *testing.T) {
		t.Parallel()
		tmpDir := t.TempDir()

		// Create two files with same basename in different directories.
		for _, rel := range []string{"pkg/a/handlers.go", "pkg/b/handlers.go"} {
			full := filepath.Join(tmpDir, rel)
			if err := os.MkdirAll(filepath.Dir(full), 0755); err != nil {
				t.Fatal(err)
			}
			if err := os.WriteFile(full, []byte("pkg "+filepath.Dir(rel)), 0644); err != nil {
				t.Fatal(err)
			}
		}
		...
		// Both should use FULL relative paths to prevent false proximity.
		if !strings.Contains(result, "pkg/a/handlers.go") {
			t.Errorf("expected 'pkg/a/handlers.go', got:\n%s", result)
		}
		if !strings.Contains(result, "pkg/b/handlers.go") {
			t.Errorf("expected 'pkg/b/handlers.go', got:\n%s", result)
		}
	})

	t.Run("NonCollidingPreservePath", func(t *testing.T) {
		t.Parallel()
		tmpDir := t.TempDir()

		// Create two files with DIFFERENT basenames in different directories.
		for _, rel := range []string{"api/server.go", "db/client.go"} {
			full := filepath.Join(tmpDir, rel)
			if err := os.MkdirAll(filepath.Dir(full), 0755); err != nil {
				t.Fatal(err)
			}
		...
		// Non-colliding basenames should still use full relative paths.
		if !strings.Contains(result, "api/server.go") {
			t.Errorf("expected 'api/server.go', got:\n%s", result)
		}
		if !strings.Contains(result, "db/client.go") {
			t.Errorf("expected 'db/client.go', got:\n%s", result)
		}
	})

	t.Run("RootLevelFiles_NoLCA", func(t *testing.T) {
		t.Parallel()
		tmpDir := t.TempDir()

		// Files at root level — no common directory.
		...
		cm, _ := NewContextManager(tmpDir)
		for _, name := range []string{"main.go", "go.mod"} {
			if err := cm.AddPath(filepath.Join(tmpDir, name)); err != nil {
				t.Fatal(err)
			}
		}
		...
		// No LCA for root-level files.
		if strings.Contains(result, "common path:") {
			t.Errorf("expected no common path for root-level files, got:\n%s", result)
		}

		// Files should appear by basename.
		if !strings.Contains(result, "-- main.go --") {
			t.Errorf("expected 'main.go', got:\n%s", result)
		}
		if !strings.Contains(result, "-- go.mod --") {
			t.Errorf("expected 'go.mod', got:\n%s", result)
		}
	})

	t.Run("TrackedDirectoriesEmitted", func(t *testing.T) {
		t.Parallel()
		tmpDir := t.TempDir()
		...
		cm, _ := NewContextManager(tmpDir)
		if err := cm.AddPath(dir); err != nil {
			t.Fatal(err)
		}

		result := cm.GetTxtarString()

		// Tracked directories should appear in comment.
		if !strings.Contains(result, "tracked directories:") {
			t.Errorf("expected tracked directories in output, got:\n%s", result)
		}
	})
}

// TestComputePathLCA verifies the LCA helper directly.
func TestComputePathLCA(t *testing.T) {
	...
	tests := []struct {
		...
		{"empty", nil, ""},
		{"single file at root", []string{"main.go"}, ""},
		{"single file in dir", []string{"src/main.go"}, "src"},
		{"same dir", []string{"src/a.go", "src/b.go"}, "src"},
		{"different dirs same parent", []string{"src/a/x.go", "src/b/y.go"}, "src"},
		{"no common dir", []string{"a/x.go", "b/y.go"}, ""},
		{"deep common", []string{"a/b/c/x.go", "a/b/d/y.go"}, filepath.Join("a", "b")},
		{"all root level", []string{"x.go", "y.go"}, ""},
	}

	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			got := computePathLCA(tc.paths)
			if got != tc.want {
				t.Errorf("computePathLCA(%v) = %q, want %q", tc.paths, got, tc.want)
			}
		})
	}
}
