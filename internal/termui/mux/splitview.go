...
func (s *SplitView) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case splitViewOutputMsg:
		// Trigger re-render when output is appended.
		return s, nil
	}
	return s, nil
}
...
func (s *SplitView) View() string {
	...
	// Calculate pane heights. Reserve 1 row for separator.
...
// renderSeparator creates the status/separator bar between panes.
...
func renderPane(lines []string, height, width int) string {
	...
	// Pad with empty lines if content shorter than pane.
