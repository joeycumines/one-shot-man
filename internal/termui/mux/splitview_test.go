...
func TestSplitView_ConcurrentAppend(t *testing.T) {
	t.Parallel()
	sv := NewSplitView(WithMaxLines(100))

	done := make(chan struct{})
	go func() {
		for i := 0; i < 100; i++ {
			sv.AppendOsmOutput("osm line")
		}
		done <- struct{}{}
	}()
	go func() {
		for i := 0; i < 100; i++ {
			sv.AppendClaudeOutput("claude line")
		}
		done <- struct{}{}
	}()
	<-done
	...
	// Just verify no panic — exact counts are non-deterministic with capping.
	if len(sv.osmLines) > 100 {
		t.Errorf("osmLines = %d, should be capped at 100", len(sv.osmLines))
	}
	if len(sv.claudeLines) > 100 {
